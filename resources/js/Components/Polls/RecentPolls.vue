...
<template>
  <div class="rounded-3xl border border-zinc-800 bg-zinc-950/60 p-4 shadow-sm">
    <div class="mb-3 flex items-center justify-between">
      ...
      <Link v-if="viewAllHref" :href="viewAllHref" class="text-xs text-zinc-400 hover:text-emerald-400 transition">
        View all
      ...
      <div v-for="poll in polls" :key="poll.id"
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            ...
            <p class="mt-1 text-[11px] text-zinc-500" v-if="showOwner">
        ...
        <div class="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-zinc-400">
          <span>
            rounds: <span class="text-zinc-200">{{ poll.rounds_count ?? 0 }}</span>
          </span>
